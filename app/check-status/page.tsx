...
export default function CheckStatusPage() {
  ...
  const handleSearch = async () => {
    ...
    // Already done — show immediately, no polling needed
    if (isTerminal(initial.status)) {
      setLoading(false);
      return;
    }

    // Still processing — poll every 1.5s until terminal
    let attempts = 0;
  ...
  const cfg = result
    ? (STATUS_CONFIG[result.status] ?? STATUS_CONFIG.PROCESSING)
    : null;
    ...
    <>
      ...
      <div className="page">
        ...
        <div className="card">
          ...
          {/* Polling progress bar — only shown while waiting */}
          {loading && isPending && (
